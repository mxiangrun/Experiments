...
class RESCAL(torch.nn.Module):
    def __init__(self, num_e: int, num_r: int, emb_dim: int = 50):
        super().__init__()
        self.e_embed = torch.nn.Embedding(num_e, emb_dim)
        self.r_embed = torch.nn.Embedding(num_r, emb_dim * emb_dim)
        self.num_e = num_e
    ...
    def to_device(self, x):
        return x.to(device=next(self.parameters()).device)

    def score(self, triplet):
        triplet = self.to_device(triplet)
        h_idx, r_idx, t_idx = triplet[:, 0], triplet[:, 1], triplet[:, 2]
        r_idx -= self.num_e
        h_emb = self.e_embed(h_idx)
        t_emb = self.e_embed(t_idx)
        r_emb = self.r_embed(r_idx)

        r_emb = r_emb.view(-1, self.emb_dim, self.emb_dim)
        score = torch.einsum("bd,bde,be->b", h_emb, r_emb, t_emb)
        return score

    def loss(self, pos_score, neg_score):
        target = torch.ones_like(pos_score)
        return self.MarginRankingLoss(pos_score, neg_score, target)
...
def load_RESCAL(dataset, config, weight_path, config_path):
    ...
    model = RESCAL(num_e=dataset.num_e,
                   num_r=dataset.num_r,
...
def save_RESCAL(model, config, weight_path, config_path):
...
class Pipeline:
    def __init__(self, config):
        ...
        self.model = RESCAL(num_e=self.dataset.num_e,
    ...
    @torch.no_grad()
    def test(self):
        self.model = load_RESCAL(dataset=self.dataset,
                                 config=self.config,
                                 weight_path=self.config.weight_path,
                                 config_path=self.config.config_path)
        self.model.eval()
...
config = SimpleNamespace(emb_dim=50,
                         ...
                         weight_path="./RESCAL_Nations_weight.pth",
                         config_path="./RESCAL_Nations_config.pkl",
                         logger_path="./RESCAL_Nations_logger.txt",
                         result_path="./RESCAL_Nations_result.xlsx", )
